...
class Calculator extends React.Component {
  ...
  handleRentRangeValueChange(e, idx, updateContext) {
    ...
    this.setState({ rentRanges: t });
    const temp = calculateTotalAmountOwedToTenant(
      t,
      this.state.cpi,
      updateContext
    );
  ...
  render() {
    ...
    const { rentRanges, zip = "" } = this.state;
    const that = this;
        ...
        {({ appCtx, updateContext }) => (
          <div className="calculator-container">
            ...
            <div className="calculator-description">
              ...
              <p>
                Renters eligible for protection under the Tenant Protection Act
                are protected against rent increases that exceed 10% in a one
                year period or the cost of living + 5%, whichever is lower. If
                you have received a rent increase you can use our calculator to
                help you determine what the allowable increase is under the law,
                and if your rent increase exceeds the limit. Eligible renters
                who got a rent increase anytime on or after March 15, 2019
                should use the rent calculator, as increases in 2019 may be
                rolled back resulting in a rent reduction.
              </p>
            ...
            <div className="card">
              <div className="card-body">
                ...
                <input
                  className="form-control"
                  type="text"
                  onChange={e => this.setCpiFromZip(e, updateContext)}
                ...
                {this.state.town && regulatedCities[this.state.town] ? (
                  <div>
                    <br />
                    <p>
                      <strong>DISCLAIMER</strong> Since you live in{" "}
                      {this.state.town} and {this.state.town} has local rent
                      control laws these calculations may not apply to you.
                      Please check local laws are not applicable to you before
                      using these state-wide calculations.
                    </p>
                  </div>
                ) : this.state.county &&
                  regulatedCounties[this.state.county] ? (
                  <div>
                    <br />
                    <p>
                      <strong>DISCLAIMER</strong> If you live in unincorporated{" "}
                      {this.state.county} county since {this.state.county}{" "}
                      county has local rent control laws these calculations may
                      not apply to you. Please check local laws are not
                      applicable to you before using these state-wide
                      calculations.
                    </p>
                  </div>
                ) : (
            ...
            <ul className="calculator-results">
              <li>
                ...
                {this.state.cpiSelection ? (
                  <h3>
                    {parseFloat(
                      (0.05 + parseFloat(this.state.cpi)) * 100
                    ).toFixed(2)}
                    %
                  </h3>
                ) : (
            ...
            {maxRent ? <AutoSubmit pageName="calculatorAutoSubmit" /> : <div />}
            <Disclaimer />
            <br />
            <br />
                ...
                <div className="card">
                  <div className="card-body">
                    <h5 className="card-title">
                      Enter your rent history from January 1st, 2020 to now.
                    </h5>
                    <section className="rent-increases">
                      <ul>{rentRangeList}</ul>
                      <SuccessButton
                        className="add"
                        onClick={this.addRentRange}
                      >
                        +
                      </SuccessButton>
                    </section>
